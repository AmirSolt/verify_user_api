...
const sendEmail: FastifyPluginAsync = async (fastify, opts): Promise<void> => {
  ...
  fastify.post<{ Headers:HeadersType, Body:BodyType, Reply:IReply }>(
    ...
    async  (request, reply) => {
      ...
      const {to_email, app_name, webhook_url, webhook_secret_key, redirect_url, extra_json} = request.body
      const verifToken = await fastify.verificationManager.saveVerificationToken(null, to_email, webhook_url, webhook_secret_key, redirect_url, extra_json)
